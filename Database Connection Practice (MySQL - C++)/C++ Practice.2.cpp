...
class DATABASE_HANDLE {
    ...
    private:
        ...
        int order_type_size = 0; //This variable possesses the amount of character of the order_type
    ...
    public:
        ...
        std::string* Order_Cols_Names()  {
            ...
            if (special_order_type != nullptr) {
                for (int i = order_type_size; i < comv.size() + order_type_size; i++) {
                    std::transform(comv.begin() + order_type_size, comv.end(), comv.begin() + order_type_size, ::toupper);
                }
        ...
        void order_col_name_char_count(std::string* comv) {

            int after_loop_pos = NULL;
            ...
            //2nd Method
            std::transform(col_names.begin(), col_names.end(), col_names.begin(), ::toupper);

            for (int i = 0; i < comv->size() + order_type_size; i++) {
                if (*(comv+i) == col_names[][i]) {
            ...
            if (after_loop_pos != NULL) {
                if (comv->size()!= after_loop_pos) {
                    std::cout << "Next 3 Characters " << std::endl;
                    std::cout << "1st : " << *(comv + after_loop_pos + 1) << std::endl;
                    std::cout << "2nd : " << *(comv + after_loop_pos + 2) << std::endl;
                    std::cout << "3rd : " << *(comv + after_loop_pos + 3) << std::endl;
                }
            }
            else {
